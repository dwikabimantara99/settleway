...
4. **Funding Gate Integrity:** Updated uyer-deposit and seller-deposit API routes to gracefully reject signing attempts for demo fallback identities without leaking secrets, Demo role Profile Wallets are recognized for local funnel testing. If server signing/runtime is not configured, funding execution fails closed. No deposit is considered funded unless a confirmed Testnet transaction is recorded.
