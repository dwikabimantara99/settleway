...
async function appendPage(input: { repository: IRepository; adminWriter: ICustodyV2AdminWriter;
  contractId: string;
  ...
  for (const eventInput of input.events) {
    ...
    const result = await input.adminWriter.appendCustodyEvent(event);
    if (result.appended) appended += 1;
  }
  return appended;
}

export async function ingestCustodyV2Events(input: { repository: IRepository; adminWriter: ICustodyV2AdminWriter;
  contractId: string;
  events: readonly NormalizedCustodyV2EventInput[];
  now?: Date;
}) {
...
export async function pollAndIngestCustodyV2Events(input: { repository: IRepository; adminWriter: ICustodyV2AdminWriter;
  rpcUrl: string;
  contractId: string;
  startLedger?: number;
  limit?: number;
  ...
  const existingCursor = await input.adminWriter.getCustodyEventCursor('testnet', input.contractId);
  ...
  while (pages < maxPages) {
    ...
    await input.adminWriter.upsertCustodyEventCursor(mergeCursor({
