...
import { applyChainCustodyProjection, assertChainDealMatchesLink } from './projection';
...
function assertActionEligibility(
  ...
  if (action === 'CREATE_DEAL' && !isBuyer) {
    throw new Error('Only the buyer may start the protected funding flow.');
  }
...
export async function prepareCustodyV2Operation(input: {
  ...
  if (input.contractReader) {
    if (input.actionType === 'CREATE_DEAL') {
      ...
      if (exists.value) {
        ...
        if (!chainDeal.ok) {
          throw new Error(`Custody V2 direct deal read failed after existence check: ${chainDeal.message}`);
        }
        await applyChainCustodyProjection({
        ...
        throw new Error('Protected Stellar deal already exists. Local Deal Room state has been reconciled.');
      }
    } else {
      const chainDeal = await input.contractReader.getDeal(input.actorAddress, link.contract_deal_id);
  ...
  const stored = await input.adminWriter.createCustodyOperation(operation);
  const returnOperation =
    !stored.created && (stored.operation.status === 'failed' || stored.operation.status === 'expired')
      ? await input.adminWriter.updateCustodyOperation(operation.idempotency_key, {
        status: 'prepared',
        rpc_result_category: null,
        confirmed_ledger: null,
        failure_code: null,
        transaction_hash: null,
        unsigned_transaction_xdr: operation.unsigned_transaction_xdr,
        prepared_transaction_body_fingerprint: operation.prepared_transaction_body_fingerprint,
        prepared_expires_at: operation.prepared_expires_at,
        updated_at: operation.updated_at,
      })
      : stored.operation;
  if (!returnOperation) {
    throw new Error('Custody V2 operation persistence failed.');
  }
  return {
    operation: returnOperation,
    unsigned_xdr: returnOperation.unsigned_transaction_xdr,
    network_passphrase: input.config.networkPassphrase,
    source_address: input.actorAddress,
    summary: {
      network: 'Stellar Testnet',
      ...
      expires_at: returnOperation.prepared_expires_at,
    },
  };
}
