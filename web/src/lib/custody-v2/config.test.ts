...
const mediatorAddress = StrKey.encodeEd25519PublicKey(Buffer.alloc(32, 23));
...
function env(overrides: Record<string, string | undefined> = {}): NodeJS.ProcessEnv {
  return {
    ...
    NEXT_PUBLIC_CUSTODY_V2_MEDIATOR_ADDRESS: mediatorAddress,
...
describe('Custody V2 config', () => {
  it('returns disabled safe defaults when the rail is explicitly disabled', () => {
    expect(loadCustodyV2PublicConfig({ NEXT_PUBLIC_CUSTODY_V2_ENABLED: 'false' }, 'demo')).toMatchObject({
      enabled: false,
      network: 'testnet',
  ...
  it('uses founder-approved public Testnet defaults in local demo mode', () => {
    ...
    expect(loadCustodyV2ServerConfig({}, 'demo').rpcUrl).toBe('https://soroban-testnet.stellar.org');
  });
  ...
  it('fails closed on missing, wrong-network, or wrong-version config', () => {
    expect(() => loadCustodyV2PublicConfig(env({ NEXT_PUBLIC_CUSTODY_V2_CONTRACT_ID: undefined }), 'persistent'))
