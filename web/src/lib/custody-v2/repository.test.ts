...
describe('Custody V2 repository boundary', () => {
  ...
  it('stores idempotent links, operations, events, and cursor state', async () => {
    ...
    expect(await repository.createCustodyDealLink(link)).toMatchObject({ created: true });
    expect(await repository.createCustodyDealLink(link)).toMatchObject({ created: false });
    expect(await repository.updateCustodyDealLink(link.application_deal_id, {
      latest_contract_state: 'AwaitingFunding',
    })).toMatchObject({ latest_contract_state: 'AwaitingFunding' });

    expect(await repository.createCustodyOperation(operation)).toMatchObject({ created: true });
    expect(await repository.createCustodyOperation(operation)).toMatchObject({ created: false });
    expect(await repository.updateCustodyOperation(operation.idempotency_key, {
      status: 'submitted',
    ...
    expect(await repository.appendCustodyEvent(event)).toMatchObject({ appended: true });
    expect(await repository.appendCustodyEvent(event)).toMatchObject({ appended: false });
    expect(await repository.listCustodyEvents(link.contract_deal_id)).toHaveLength(1);

    expect(await repository.upsertCustodyEventCursor(cursor)).toMatchObject(cursor);
    expect(await repository.getCustodyEventCursor('testnet', link.contract_id)).toMatchObject(cursor);
  });
});
