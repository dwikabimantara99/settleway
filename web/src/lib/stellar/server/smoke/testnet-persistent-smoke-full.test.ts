...
import { redact } from '../../../../../scripts/testnet-persistent-smoke';
...
import { runSmoke } from '../../../../../scripts/testnet-persistent-smoke-full';
...
describe('Testnet Persistent Smoke Runner & Hook', () => {
  ...
  it('hook preserves idempotency key usage', async () => {
    ...
    mockCoordinateDealExecution.mockResolvedValue({ ok: true, current_deal: {}, next_deal: { status: 'BUYER_FUNDED' } });
  ...
  it('runner with mock executes funding and returns safe metadata', async () => {
    ...
    const dealStatus = 'WAITING_DEPOSITS';
    ...
    mockGetDeal.mockImplementation((id: string) => Promise.resolve({ 
       ...
       stellar_mode: 'testnet', status: dealStatus, 
    ...
    mockCoordinateDealExecution
      .mockResolvedValueOnce({ ok: true, current_deal: {}, next_deal: { status: 'BUYER_FUNDED' } })
      .mockResolvedValueOnce({ ok: true, current_deal: {}, next_deal: { status: 'LOCKED' } })
      .mockResolvedValueOnce({ ok: true, current_deal: {}, next_deal: { status: 'PROOF_SUBMITTED' } })
      .mockResolvedValueOnce({ ok: true, current_deal: {}, next_deal: { status: 'DELIVERED' } })
      .mockResolvedValueOnce({ ok: true, current_deal: {}, next_deal: { status: 'COMPLETED' } });
    ...
    expect(report.classification).toBe('FULL_LIFECYCLE_LOCAL_SMOKE_SUCCEEDED');
