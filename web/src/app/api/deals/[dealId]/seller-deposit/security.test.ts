...
describe('Seller Deposit Security Boundaries', () => {
  ...
  it('Privileged repository is not constructed for the wrong funding role (buyer)', async () => {
    vi.mocked(requireDealParticipant).mockResolvedValueOnce({
      deal: { id: 'deal-1', status: 'BUYER_FUNDED' } as unknown,
      role: 'buyer',
      user: { id: 'buyer-1' } as unknown,
    });
    ...
    expect(data.error.message).toBe('Only seller can perform this action');
    expect(createPrivilegedServerRepository).not.toHaveBeenCalled();
  });
});
