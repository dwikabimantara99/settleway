...
import { POST as refundRoute } from "./route";
...
describe("refund route", () => {
  ...
  it("records testnet-backed neutral refund with public transaction anchoring", async () => {
    setupDeal("deal-refund-testnet");
    vi.mocked(nextHeaders.cookies).mockReturnValue({
      get: () => ({ value: "buyer-1" }),
    } as any);

    const response = await refundRoute(
      new Request("http://localhost/api/deals/deal-refund-testnet/refund", {
        method: "POST",
      }),
      { params: Promise.resolve({ dealId: "deal-refund-testnet" }) },
    );

    expect(response.status).toBe(200);
    expect(mockStore.deals.get("deal-refund-testnet")?.status).toBe("REFUNDED");
    expect(mockStore.deals.get("deal-refund-testnet")?.latest_stellar_tx_hash).toBe("a".repeat(64));

    const roomEvents = mockStore.getDealEvents("deal-refund-testnet");
    ...
    const reputationEvents = mockStore.getDealReputationEvents("deal-refund-testnet");
    expect(reputationEvents).toHaveLength(2);
    expect(reputationEvents.every((event) => event.reputation_outcome === "refunded_before_locked")).toBe(true);
